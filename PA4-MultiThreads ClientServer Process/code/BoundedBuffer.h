...
public:
	...
	void push(char* data, int len){
		...
		//2. Then push the vector at the end of the queue, using thread-safety method
		q.push(d);
		l.unlock();
	...
	int pop(char* buf, int bufcap){
		...
		unique_lock<mutex> l (m);
		data_available.wait(l, [this]{return q.size() > 0;});	// only wakes up when there is something in the q
		
		//2. pop the front item of the queue. The popped item is a vector<char>
		// use thread-safety method
		vector<char> d = q.front();
		q.pop();
		l.unlock();	// unlock ASAP

		//3. Convert the popped vector<char> into a char*, copy that into buf, make sure that vector<char>'s length is <= bufcap
		assert(d.size() <= bufcap);			// make sure buffer does not overflow
		memcpy(buf, d.data(), d.size());	// d.data() gives you the byte sequence of the vector content
