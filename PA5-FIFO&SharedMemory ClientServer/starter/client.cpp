...
RequestChannel* create_new_channel(RequestChannel* control, string ival, int mb)
{
    // get the name 
    char name[1024];
...
*/
void worker_thread_function(RequestChannel* chan, HistogramCollection* hc, BoundedBuffer* request_buffer, int mb){
    char buf[1024];
    double resp = 0.0;
...
int main(int argc, char *argv[])
{
    ...
    srand(time_t(NULL));
    string ival = "f";  // the string val that indicates whether the request is fifo or MQ
    ...
    // create a parent class pointer
	RequestChannel* chan;
    // based on the ival, decide whether to create FIFO or MQ
    if(ival == "f")
    {
    ...
    for(int i = 0; i < w; i++)
    {
        // the create_new_channel method can return a correct channel based on ival
    ...
    // the following will not work since execlp(.) does not support wild card. 
    // int pid_1 = fork();
    // if(pid_1 == 0)
    // {
